...
PHP_INI_MH(OnUpdateForwardLimit) /* {{{ */ {
	...
	if (UNEXPECTED(limit <= 0)) {
		/* a zero limit would underflow the dispatch-loop counter and spin
		   forever; fall back to the default instead */
		limit = YAF_DEFAULT_FORWARD_LIMIT;
	}
...
PHP_INI_BEGIN()
	...
	PHP_INI_ENTRY("yaf.forward_limit",     ZEND_TOSTR(YAF_DEFAULT_FORWARD_LIMIT), PHP_INI_ALL, OnUpdateForwardLimit)
