...
yaf_route_t * yaf_route_rewrite_instance(yaf_route_t *this_ptr, zval *match, zval *route, zval *verify TSRMLS_DC) {
	...
	if (!verify || IS_STRING != Z_TYPE_P(verify)) {
...
/** {{{ zval * yaf_route_rewrite_assemble(yaf_route_t *this_ptr, zval *match_arr, zval *query TSRMLS_DC)
 */
zval * yaf_route_rewrite_assemble(yaf_route_t *this_ptr, zval *match_arr, zval *query TSRMLS_DC) {
	char    tvalue[1024];
	uint    tvalue_len = 0;
	...
	verify = zend_read_property(yaf_route_rewrite_ce, this_ptr, ZEND_STRL(YAF_ROUTE_PROPETY_NAME_VERIFY), 1 TSRMLS_CC);
	...
	do {
		...
		if (IS_ARRAY != Z_TYPE_P(match_arr)) {
			yaf_trigger_error(YAF_ERR_TYPE_ERROR TSRMLS_CC, "%s", "Expects an array as 1st parmeter");
			break;
		}

		zval *match_data, func_name;
		zval *args[2];

		MAKE_STD_ZVAL(match_data);
		MAKE_STD_ZVAL(args[0]);
		MAKE_STD_ZVAL(args[1]);
		INIT_ZVAL(func_name);

		ZVAL_STRING(args[0], Z_STRVAL_P(verify), 0);
		ZVAL_ZVAL(args[1], match_arr, 0, 0);
		ZVAL_STRING(&func_name, "vsprintf", 0);

		call_user_function(EG(function_table), NULL, &func_name,
				match_data, 2, args TSRMLS_CC);

		if (IS_STRING == Z_TYPE_P(match_data)) {
			tvalue_len = Z_STRLEN_P(match_data);
			strncpy(tvalue, Z_STRVAL_P(match_data), tvalue_len);
			zval_ptr_dtor(&match_data);
		}
...
/** {{{ proto public Yaf_Route_Rewrite::__construct(string $match, array $route, string $verify = NULL)
 */
PHP_METHOD(yaf_route_rewrite, __construct) {
	zval 		*match, *route, *verify;
	yaf_route_t	*self = getThis();

	if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "za|z", &match, &route, &verify) ==  FAILURE) {
	...
	if (ZEND_NUM_ARGS() == 3 && IS_STRING != Z_TYPE_P(verify)) {
		YAF_UNINITIALIZED_OBJECT(getThis());
...
PHP_METHOD(yaf_route_rewrite, assemble) {
	zval    *match_arr, *query, *return_uri = NULL;

	if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "z|z", &match_arr, &query) == FAILURE) {
		return;
	} else {
		return_uri = yaf_route_rewrite_assemble(getThis(), match_arr, query TSRMLS_CC);
