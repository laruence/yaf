...
`tools/yaf_classes.php` contains the full Yaf API as plain PHP signatures,
generated by reflection over the real extension (`tools/yaf.php`). Register
it as a stub / include-path entry in your IDE (PhpStorm: Settings → PHP →
Include path, or mark the file as a stub) to get code completion and
parameter hints for the C-implemented classes. Regenerate it after API
changes with:
