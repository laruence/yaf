...
`tools/ide/` ships the full Yaf API as plain PHP signatures for IDE code
completion: `yaf_classes.php` (underscore style) and
`yaf_classes_namespace.php` (for `yaf.use_namespace=1` projects). Register
the one matching your class style as a stub / include-path entry in your
IDE (PhpStorm: Settings → PHP → Include path, or mark the file as a stub)
to get completion and parameter hints for the C-implemented classes. Both
are generated by `tools/ide/yaf.php` reflecting over the real extension;
regenerate them after API changes with:

```bash
php -d extension=yaf.so tools/ide/yaf.php > tools/ide/yaf_classes.php
php -d extension=yaf.so -d yaf.use_namespace=1 tools/ide/yaf.php > tools/ide/yaf_classes_namespace.php
```
